...
static func spawn_instance(
		scene: PackedScene, target: Node2D, distance: float, parent: Node2D) -> Node2D:
...
static func spawn_instance_bounded(scene: PackedScene, target: Node2D, distance: float, parent: Node2D, mask_bit: int) -> Node2D:
...
## 'scene' instance global_position is set at random point that is 'distance' away from 'target'
## instance will be within a clear line of sight to 'target' (not intersecting 'mask_bit' objects)
static func _get_instance_bounded(scene: PackedScene, target: Node2D, distance: float, mask_bit: int) -> Node2D:
	...
	for i in 4:
		...
		var query_params: PhysicsRayQueryParameters2D = PhysicsRayQueryParameters2D.create(
			target.global_position, spawn_position, 1 << mask_bit)
